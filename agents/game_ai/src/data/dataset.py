...
def download_game_ai_dataset(**kwargs) -> Path:
    """Download HuggingFace dataset untuk domain game_ai."""
    ...
    output_path = data_dir / "game_ai_dataset.parquet"

    force_clean = kwargs.get("force", False)
    is_full = kwargs.get("full", False)

    if not output_path.exists() or force_clean:
        logger.info(f"Mengunduh dataset mateuszgrzyb/lichess-stockfish-normalized dari HuggingFace...")

        # Mengunduh dataset tanpa trust_remote_code
        ds = load_dataset("mateuszgrzyb/lichess-stockfish-normalized", token=hf_token)
