...
def download_data_engineering_dataset(**kwargs) -> Path:
    """Download HuggingFace dataset untuk domain data_engineering."""
    # Ambil Token HF dari environment (berguna untuk dataset private/gated)
    hf_token = os.environ.get("HF_TOKEN")

    data_dir = Path(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..", "..", "data", "raw", "data_engineering")))
    data_dir.mkdir(parents=True, exist_ok=True)

    output_path = data_dir / "data_engineering_dataset.parquet"

    force_clean = kwargs.get("force", False)
    is_full = kwargs.get("full", False)

    if not output_path.exists() or force_clean:
        logger.info(f"Mengunduh dataset gretelai/synthetic_text_to_sql dari HuggingFace...")

        # Mengunduh dataset tanpa trust_remote_code
        ds = load_dataset("gretelai/synthetic_text_to_sql", token=hf_token)
