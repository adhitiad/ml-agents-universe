...
def download_healthcare_dataset(**kwargs) -> Path:
    """Download HuggingFace dataset untuk domain healthcare."""
    ...
    output_path = data_dir / "healthcare_dataset.parquet"

    force_clean = kwargs.get("force", False)
    is_full = kwargs.get("full", False)

    if not output_path.exists() or force_clean:
        logger.info(f"Mengunduh dataset starmpcc/Asclepius-Synthetic-Clinical-Notes dari HuggingFace...")

        # Mengunduh dataset tanpa trust_remote_code
        ds = load_dataset("starmpcc/Asclepius-Synthetic-Clinical-Notes", token=hf_token)
