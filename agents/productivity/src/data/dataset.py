...
def download_productivity_dataset(**kwargs) -> Path:
    """Download HuggingFace dataset untuk domain productivity."""
    # Ambil Token HF dari environment (berguna untuk dataset private/gated)
    hf_token = os.environ.get("HF_TOKEN")

    data_dir = Path(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..", "..", "data", "raw", "productivity")))
    data_dir.mkdir(parents=True, exist_ok=True)

    output_path = data_dir / "productivity_dataset.parquet"

    force_clean = kwargs.get("force", False)
    is_full = kwargs.get("full", False)

    if not output_path.exists() or force_clean:
        logger.info(f"Mengunduh dataset SetFit/enron_spam dari HuggingFace...")

        # Mengunduh dataset tanpa trust_remote_code
        ds = load_dataset("SetFit/enron_spam", token=hf_token)
