...
def download_devops_dataset(**kwargs) -> Path:
    """Download HuggingFace dataset untuk domain devops."""
    # Ambil Token HF dari environment (berguna untuk dataset private/gated)
    hf_token = os.environ.get("HF_TOKEN")

    data_dir = Path(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..", "..", "data", "raw", "devops")))
    data_dir.mkdir(parents=True, exist_ok=True)

    output_path = data_dir / "devops_dataset.parquet"

    force_clean = kwargs.get("force", False)
    is_full = kwargs.get("full", False)

    if not output_path.exists() or force_clean:
        logger.info(f"Mengunduh dataset RazinAleks/SO-Python_QA-System_Administration_and_DevOps_class dari HuggingFace...")

        # Mengunduh dataset tanpa trust_remote_code
        ds = load_dataset("RazinAleks/SO-Python_QA-System_Administration_and_DevOps_class", token=hf_token)
